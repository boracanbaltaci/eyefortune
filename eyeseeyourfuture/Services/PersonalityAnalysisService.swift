...
enum PersonalityAnalysisService {
    ...
    static func generateReading(for iris: IrisInfo, userName: String) -> String {
        ...
        let readings = [
            "blue": """
                Gözlerindeki derinlik ve \(iris.pattern) deseni, ruhunun engin bir okyanus gibi dingin ama bir o kadar da derin olduğuna işaret ediyor, \(name). 
                ...
                """,
            "amber": """
                Bu sıcaklık ve \(iris.pattern) dokusu, içindeki sönmeyen 'Kehribar' ateşini temsil ediyor, \(name). 
                ...
                """,
            "green": """
                Gözlerindeki yansımalar ve \(iris.pattern) karmaşıklığı, doğanın yenileyici ve şifacı gücünü taşıdığını fısıldıyor, \(name).
                ...
                """,
            "mystic": """
                Bu gizem ve \(iris.pattern) belirsizliği, senin evrenin en nadide elementlerinden biri olduğunu gösteriyor, \(name).
